"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
/**
 * Created by frontend-huan on 2016/11/26.
 */
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var houses_list_component_1 = require("./houses-list.component");
var house_detail_component_1 = require("./house-detail.component");
var housesRoutes = [
    { path: 'houses', component: houses_list_component_1.HousesListComponent },
    { path: 'house/:id', component: house_detail_component_1.HouseDetailComponent }
];
var HousesRoutingModule = (function () {
    function HousesRoutingModule() {
    }
    HousesRoutingModule = __decorate([
        core_1.NgModule({
            imports: [
                router_1.RouterModule.forChild(housesRoutes)
            ],
            exports: [
                router_1.RouterModule
            ]
        }), 
        __metadata('design:paramtypes', [])
    ], HousesRoutingModule);
    return HousesRoutingModule;
}());
exports.HousesRoutingModule = HousesRoutingModule;
//# sourceMappingURL=houses-routing.module.js.map